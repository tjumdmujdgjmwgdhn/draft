...
<!--文件体开始-->
	...
	<form action="login.action" method=post name="login">
		...
		<td valign=middle class=tablebody1>请输入您的用户名</td>
			<td valign=middle class=tablebody1><INPUT name=userid id="username" type=text> &nbsp;
				<a href="ToRegister">没有注册？</a>
			</td>
		</tr>
		<tr>
			<td valign=middle class=tablebody1>请输入您的密码</td>
			<td valign=middle class=tablebody1><INPUT name=password id="password" type=password> &nbsp; </td>
		</tr>
		<tr>
			<td class=tablebody2 valign=middle colspan=2 align=center><input type=button value="登 录" onclick = "return checkUserName()" ></td><!-- onclick="javascript:checkMe()" -->
		</tr>
		</table>
		<%-- <c:choose>
			<c:when test="${user.userid !=userid || user.password != password }">submit onblur="checkUserName()" 
				<script type="text/javascript">
					function check (){
						alert("用户名与密码不一致！");
						return false;
		...
		function checkUserName() { 
			...
			// 调用Ajax函数,向服务器端发送查询  
		  	 $.ajax({ //一个Ajax过程  
		      	...
		      	url :"checkUserName", //与此页面沟通  "register.action"
		      	dataType:'json',					//返回的值以 JSON方式 解释  
		      	data: 'userid='+username,	 		//	obj.value, 发给的数据  
		      	success: function(json){//如果调用成功  
		      	if(json.flag != password){     
		    	   		alert("用户名与密码不一致，请检查 ");
		    	   		//alert(json.password);
		    	   		return false;
		          	}else{
