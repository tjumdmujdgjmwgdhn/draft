...
<!--文件体开始-->
        ...
        <c:forEach items="${orderlines }" var="orderline" varStatus="vs">    
		<tr>
			<form method="post" action="updateProduct" name="f1">
			<input type="hidden" name="productid" value="${orderline.product.productid }"> 
			<input type="hidden" name="number" value="${orderline.amount }"> <%-- ${orderline.amount } --%>
			<td class=tablebody2 valign=middle align=center width="">${vs.count }</td><%--   --%>
			<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="productDetail.jsp?productid=${orderline.product.productid  }">${orderline.product.name }</a></td>
			<td class=tablebody2 valign=middle align=center width="">${orderline.product.baseprice }</td>
			<td class=tablebody1 valign=middle align=center width=""><input type="text" name="num" value="${orderline.amount }" size="4" onblur="javascript:if(this.value<1){alert('对不起，产品数量不能小于 1 ');this.focus();}else{number.value=this.value;}"/></td>
			<td class=tablebody2 valign=middle align=left width="">&nbsp;&nbsp;￥${orderline.product.baseprice * orderline.amount }</td><%-- ${orderline.baseprice * orderlines.size() } --%>
			<td class=tablebody1 valign=middle align=center width="">
			<input type="button" value="取消" onclick="javascript:window.location='removeProduct?productid=${orderline.product.productid }';"> <input type="submit" value="保存修改"></td>
			</form>
		</tr>
        </c:forEach>           
		<tr>
			<td class=tablebody1 valign=middle align=center colspan="4">　</td>
			<td class=tablebody1 valign=middle align=left width="">&nbsp;&nbsp;<font color="#ff0000"><b>￥${totalPrice}</b></font></td><!-- {sessionScope.cart.totalPrice} -->
			<td class=tablebody1 valign=middle align=center width="">　</td>
		</tr>
		<tr>
			<td class=tablebody2 valign=middle align=center colspan="6"><input type="button" value="提交订单" onclick="javascript:window.location='user/confirmOrder.jsp';"> <input type="button" value="继续购物" onclick="javascript:window.location='index.jsp';"> <input type="button" value="清空购物车" onclick="javascript:window.location='removeAllProduct';"></td>
		</tr>
                </table><br>
<!--文件尾开始-->
